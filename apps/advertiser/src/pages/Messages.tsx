...
import { demoAdvertiser, getOwnThread, markThreadRead, sendMessage, type OwnThread } from '@cpatracker/mock';
...
export function Messages() {
  ...
  async function load() {
    const own = await getOwnThread(demoAdvertiser.id);
    setThread(own);
    const hasUnread = own.messages.some((m) => m.recipientId === demoAdvertiser.id && m.readAt === null);
    if (hasUnread) await markThreadRead(own.threadId, demoAdvertiser.id);
  }
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      await sendMessage(thread.threadId, thread.otherPartyUserId, reply.trim(), demoAdvertiser.id);
      setReply('');
      await load();
      ...
      <div className="flex h-[60vh] flex-col gap-4 rounded-lg border border-border bg-card p-4">
        <div className="flex-1 space-y-3 overflow-y-auto">
          ...
          {thread?.messages.map((message) => {
            const fromMe = message.senderId === demoAdvertiser.id;
            return (
              <div
                key={message.id}
                className={
