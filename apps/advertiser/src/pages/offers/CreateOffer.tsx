...
import { createOffer, createOfferCategory, demoAdvertiser, getOfferCategories } from '@cpatracker/mock';
import type { OfferCap, TrackingPlatform, TrafficType } from '@cpatracker/types';
...
export function CreateOffer() {
  ...
  const valid = name.trim().length > 0 && trafficTypes.length > 0 && payoutRules.length > 0;
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        New offers are submitted as pending and go live once the network admin approves them.
      </p>
      ...
      <section className="space-y-4 rounded-lg border border-border bg-card p-4">
        ...
        <div className="space-y-1.5">
          <label className="text-sm font-medium text-foreground">Tracking Platform</label>
          <Select
            ...
            className="max-w-sm"
          />
          <p className="text-xs text-muted-foreground">Choose your tracking software or network name.</p>
        </div>
      ...
      <ImportOffersModal open={importOpen} onOpenChange={setImportOpen} onImported={() => navigate('/offers')} />
    </div>
  );
}
