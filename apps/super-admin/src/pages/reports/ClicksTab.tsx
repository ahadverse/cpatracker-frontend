...
import { getAffiliates, getClicks, getOffers, getSmartLinks, type ClickFilters } from '@cpatracker/mock';
import type { Affiliate, Click, ClickOS, Offer, SmartLink } from '@cpatracker/types';
...
export function ClicksTab() {
  ...
  useEffect(() => {
    Promise.all([getOffers(), getAffiliates(), getSmartLinks()]).then(([offerRows, affiliateRows, smartLinkRows]) => {
      setOffers(offerRows);
      setAffiliates(affiliateRows);
      setSmartLinks(smartLinkRows);
    });
  }, []);
