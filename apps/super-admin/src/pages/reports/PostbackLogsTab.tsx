...
import {
  getNetworkAdminConversions,
  getAdvertisers,
  getAffiliates,
  getOffers,
  getPostbackLogs,
} from '@cpatracker/mock';
import type { NetworkAdminConversion, Advertiser, Affiliate, Offer, PostbackLog } from '@cpatracker/types';
...
export function PostbackLogsTab({ dimension }: PostbackLogsTabProps) {
  ...
  useEffect(() => {
    Promise.all([getPostbackLogs(), getOffers(), getNetworkAdminConversions(), getAffiliates(), getAdvertisers()]).then(
