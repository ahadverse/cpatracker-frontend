...
import { createOffer } from '@cpatracker/mock';
...
export function ImportOffersModal({ open, onOpenChange, advertiserId, onImported }: ImportOffersModalProps) {
  ...
  async function handleImport() {
    if (!advertiserId || rows.length === 0) return;
    setImporting(true);
    try {
      await Promise.all(
        rows.map((row) => {
      ...
      toast.success(`Imported ${rows.length} offer(s)`);
      setRows([]);
      onOpenChange(false);
      onImported();
    } finally {
    ...
    <Modal open={open} onOpenChange={onOpenChange} title="Import offers">
      <div className="space-y-4">
        <p className="text-xs text-muted-foreground">
          ...
          traffic type codes). First row is treated as a header and skipped.
        </p>

        {!advertiserId && (
          <p className="text-sm text-warning">Select an advertiser in the form below before importing.</p>
        )}
        ...
        <button
          type="button"
          disabled={!advertiserId || rows.length === 0 || importing}
