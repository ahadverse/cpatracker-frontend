...
import { getAdminConversions, getAdvertisers, getAffiliates, getOffers } from '@cpatracker/mock';
import type { AdminConversion, Advertiser, Affiliate, Offer } from '@cpatracker/types';
import { DataTable, StatusBadge } from '@cpatracker/ui';

const STATUS_VARIANT: Record<AdminConversion['status'], 'success' | 'warning' | 'destructive'> = {
  APPROVED: 'success',
  PENDING: 'warning',
...
export function ConversionsTab() {
  ...
  useEffect(() => {
    Promise.all([getAdminConversions(), getOffers(), getAffiliates(), getAdvertisers()]).then(
  ...
  const columns: ColumnDef<AdminConversion>[] = [
